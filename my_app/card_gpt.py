...
for message in st.session_state["messages"]:
    with st.chat_message(message["role"]): # chat_message: 메시지의 발신자 role(assistant인지 user인지)에 따라 UI를 구분하여 메시지 창을 표시해주는 함수 
...
# 4. ai 답변 생성 & 출력
if st.session_state["messages"][-1]["role"] != "assistant": # messages 리스트에 담긴 메세지가 ai가 아닌 경우
